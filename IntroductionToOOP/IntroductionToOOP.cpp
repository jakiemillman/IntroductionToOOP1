...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	                MOVE Методы!!!!!
		При использовании в классе динамической памяти, копирование объектов может занимать достаточно много времени и ресурсов, как в памяти, так и процессорного времени. Особенно это
	бессмысленно при копировании временных безымянных объектов, которые существуют только в пределах одного выражения. Временный безымянный объект всякий раз неявно создаётся, когда функция 
	возвращает результат своей работы по значению. В этом есть необходимойсть когда из функции возвращается локальная переменная. Локальную переменную нельзя вернуть по ссылку, посколько она будет удалена 
	из памяти по завершению функции. Для того, чтобы сэкономить ресурсы затрачиваемые на копирование временного безымянного объекта, эти объекты можно перемещать на место вызова. 
	Зачем копировать то, вчто будет удалено. Для этого существуют методы переноса. К ним относятся конструктор переноса, и оператор присваивания переноса. Методы переноса
	принимают r-value reference(Class&& obj). Они переносят память переносимого объекта на место вызова, а именно выполняют Shalow Copy. После поверхностного копирования move метод должен онулить
	другой объект. Плскольку для объекта будет вызван деструктор, и он не должен удалить динамическую память этого объекта, поскольку она присвоина другому объекту.
			*/